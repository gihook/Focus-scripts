...
from src.utils import (
    format_workflow_steps_ascii, select_user, print_user_header, clear_screen,
    print_app_header, print_app_footer, load_config, extract_xsrf_token
)
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='?',
        help="Optional Submission UUID or Display ID to view details directly"
    )
...
def main():
    ...
    if direct_id is not None:
        ...
        if uuid_pattern.match(direct_id):
            submission_uuid = direct_id
        else:
            print(f"Resolving UUID for display ID '{direct_id}'...")
            search_url = f"{host}/submissions/search?searchTerm={urllib.parse.quote(direct_id)}&pageNumber=1&pageSize=10"
            res_data = make_get_request(search_url)
            search_items = []
            if isinstance(res_data, list):
                search_items = res_data
            elif isinstance(res_data, dict):
                search_items = res_data.get('items', res_data.get('results', res_data.get('data', [])))

            for item in search_items:
                if str(item.get('displayId')) == direct_id or str(item.get('title')).lower() == direct_id.lower() or str(item.get('subject')).lower() == direct_id.lower():
                    submission_uuid = item.get('id')
                    break
            if not submission_uuid and search_items:
                submission_uuid = search_items[0].get('id')

        if not submission_uuid:
            print(f"Error: Could not find submission with ID '{direct_id}'.")
            sys.exit(1)

        items = [{"id": submission_uuid, "displayId": direct_id}]
        should_fetch = False
        choice = "1"
    ...
    while True:
        ...
        if choice is None:
            ...
            choice_lower = raw_choice.lower().strip()
        else:
