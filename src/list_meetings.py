...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs='?',
        help="Optional Meeting UUID or Display ID to view details directly"
    )
...
def main():
    ...
    if direct_id is not None:
        ...
        if uuid_pattern.match(direct_id):
            meeting_uuid = direct_id
        else:
            print(f"Resolving UUID for display ID '{direct_id}'...")
            search_url = f"{host}/meetings/search?searchTerm={urllib.parse.quote(direct_id)}&pageNumber=1&pageSize=10"
            ...
            for item in search_items:
                if str(item.get('displayId')) == direct_id or str(item.get('meetingNo')).lower() == direct_id.lower():
                    meeting_uuid = item.get('id')
                    break
            if not meeting_uuid and search_items:
                meeting_uuid = search_items[0].get('id')

        if not meeting_uuid:
            print(f"Error: Could not find meeting with ID '{direct_id}'.")
            sys.exit(1)

        items = [{"id": meeting_uuid, "displayId": direct_id}]
        should_fetch = False
        choice = "1"
